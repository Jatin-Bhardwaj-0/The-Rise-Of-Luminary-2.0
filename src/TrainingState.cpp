...
namespace Layout {
    ...
    constexpr float HUD_Y   = 18.f,   HUD_H   = 95.f;
    constexpr float PHUD_X  = 8.f,   PHUD_W  = 300.f;   // player (left)
    constexpr float SCORE_X = 484.f, SCORE_W = 220.f;   // score  (centre)
    constexpr float EHUD_X  = 878.f, EHUD_W  = 300.f;   // enemy  (right)

    // Battle log — floats just above the question box
    constexpr float BLOG_Y = 242.f;

    // Question panel — large centre decorative box
    constexpr float QBOX_X = 211.f,  QBOX_W = 800.f;
    constexpr float QBOX_Y = 235.f, QBOX_H  = 130.f;

    // Answer grid (2×2)
    constexpr float BTN_H   = 45.f;
    constexpr float BTN_GAP = 10.f;
    constexpr float BTN_W   = 461.f;
    constexpr float COL0_X  = 115.f;
    constexpr float COL1_X  = 665.f;
    constexpr float GRID_Y  = 405.f;   // 6px gap below question box bottom (441)
    constexpr float ROW2_Y  = 585.f;   // GRID_Y + BTN_H + BTN_GAP

    // Feedback strip (visible only when answered / hinted)
    constexpr float FB_Y = ROW2_Y + BTN_H + 4.f;   // 641
    constexpr float FB_H = 35.f;
...
TrainingState::TrainingState(Game* gameInstance)
    ...
    questionText->setPosition(sf::Vector2f(QBOX_X - 15.f, QBOX_Y + 16.f));
